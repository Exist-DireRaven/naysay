...
/// The TUI has no notes field, so postmortem always runs on the idea alone.
/// The CLI fills the same `{notes}` slot from its optional argument.
const NO_NOTES: &str = "The user provided no notes. Reason from the idea \
itself and common patterns, and where the real outcome matters, say what \
...
async fn run_premortem<F: FnMut(&str) + Send>(
    ...
    const DEFAULT: &str = crate::prompts::PREMORTEM;
...
async fn run_check<F: FnMut(&str) + Send>(
    ...
    const DEFAULT: &str = crate::prompts::CHECK;
...
async fn run_spec<F: FnMut(&str) + Send>(
    ...
    const DEFAULT: &str = crate::prompts::SPEC;
...
async fn run_postmortem<F: FnMut(&str) + Send>(
    ...
    const DEFAULT: &str = crate::prompts::POSTMORTEM;
    let template = prompts.get("postmortem", DEFAULT);
    let prompt = template
        .replace("{idea}", idea)
        .replace("{notes}", NO_NOTES);
